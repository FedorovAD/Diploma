...
export async function parse() {
    ...
    const website = await getHTML('https://www.afisha.ru/msk/theatre/theatre_list/');
    const pageNumber = website('.yNWhI').eq(-1).text();
    const objArr: any = [];
    for (let i = 0; i < pageNumber; i++){
        const selector = await getHTML(`https://www.afisha.ru/msk/theatre/theatre_list/page${i + 1}/`)
        selector('div._yjkz').each((i: number, element: any) => {
            ...
            const typeOfPlace: string = 'Театр'
            objArr.push({
                title,
                typeOfPlace,
                titleLink,
